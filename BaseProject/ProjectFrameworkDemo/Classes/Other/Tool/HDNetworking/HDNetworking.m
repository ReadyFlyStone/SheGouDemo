...
{
    URLString = [NSString stringWithFormat:@"%@%@", projectBaseURL, URLString];
    ...
    if ([parameters isKindOfClass:[NSMutableDictionary class]]) {
        [parameters setValue:userManager.curUserInfo.sessionId forKey:@"sessionId"];
    }
...
- (BOOL)judgeResultAndhandleWithCode:(id)responseObject {
    ...
    if ([[responseObject valueForKey:@"code"] integerValue]==2) {
        ...
        if (![vc isKindOfClass:[FWLoginVC class]]) {
            if ([vc isKindOfClass:[CYLTabBarController class]]) {
                CYLTabBarController *tabberVC = (CYLTabBarController *)vc;
                RootNavigationController *nav = (RootNavigationController *)tabberVC.viewControllers[tabberVC.selectedIndex];
                RootNavigationController *nav1 = [[RootNavigationController alloc] initWithRootViewController:[FWLoginVC new]];
                [nav presentViewController:nav1 animated:YES completion:nil];
            } else {
                RootNavigationController *nav = [[RootNavigationController alloc] initWithRootViewController:[FWLoginVC new]];
